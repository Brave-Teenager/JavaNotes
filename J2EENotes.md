...
在java web程序中，Servlet主要负责接收用户请求HttpServletRequest，在doGet(),dopost()中做相应的处理，并将回应的HttpServletResponse 反馈给用户。Servlet可以设置初始化参数，供Servlet内部使用。一个Servlet只有一个实例，在他初始化的时候回调用init（）方法，

销毁时使用destory（）方法，Servlet需在web.xml里面配置，一个Servlet可以设置多个URL参数的访问，Servlet不是线程安全的，要谨慎使用变量。
...
本质上无区别，是HTTP协议中的两种发送请求的方法。HTTP是什么？HTTP是基于TCP/IP的关于数据如何在万维网中如何通信的协议。
